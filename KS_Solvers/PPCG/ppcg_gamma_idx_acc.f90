!
SUBROUTINE ppcg_gamma_idx_acc( h_psi, s_psi, overlap, precondition, &
...
!
  ...
  DO WHILE ( ((trdif > trtol) .OR. (trdif == -1.D0))  .AND. (iter <= maxter) .AND. (nact > 0) )
     ...
     DO j = 1, nact ; DO i=1,nblock
        w(1+(i-1)*blocksz:MIN(i*blocksz,npw), j) = & 
                w(1+(i-1)*blocksz:MIN(i*blocksz,npw), j) / &
     ...
     if (overlap) then
        if (n_start .le. n_end) &
        CALL DGEMM( 'T','N', my_n, nact, npw2, 2.D0,spsi(1,n_start), npwx2, w, npwx2, 0.D0, G(n_start,1), nbnd )
        IF ( gstart == 2 ) CALL DGER( my_n, nact, -1.D0,spsi(1,n_start), npwx2, w, npwx2, G(n_start,1), nbnd )
     else
        if (n_start .le. n_end) &
        CALL DGEMM( 'T','N', my_n, nact, npw2, 2.D0, psi(1,n_start), npwx2, w, npwx2, 0.D0, G(n_start,1), nbnd )
        IF ( gstart == 2 ) CALL DGER( my_n, nact, -1.D0, psi(1,n_start), npwx2, w, npwx2, G(n_start,1), nbnd )
     end if
     ...
     IF ( my_bgrp_id /= root_bgrp_id ) call threaded_memset( w, 0.d0, 2*npwx*nact )  
     if (n_start .le. n_end) &
     CALL DGEMM('N','N', npw2, nact, my_n, -ONE, psi(1,n_start), npwx2, G(n_start,1), nbnd, ONE, w, npwx2)
...
!ev     IF ( MOD(iter, rr_step) /= 1 ) THEN    ! In this case, P is skipped after each RR
     IF ( iter  /=  1 ) THEN
        ...
        if (overlap) then
           if (n_start .le. n_end) &
           CALL DGEMM('T','N', my_n, nact, npw2, 2.D0, spsi(1,n_start), npwx2, p, npwx2, 0.D0, G(n_start,1), nbnd)
           IF ( gstart == 2 ) CALL DGER( my_n, nact, -1.D0, spsi(1,n_start), npwx2, p, npwx2, G(n_start,1), nbnd )
        else
           if (n_start .le. n_end) &
           CALL DGEMM('T','N', my_n, nact, npw2, 2.D0, psi(1,n_start), npwx2, p, npwx2, 0.D0, G(n_start,1), nbnd)
           IF ( gstart == 2 ) CALL DGER( my_n, nact, -1.D0, psi(1,n_start), npwx2, p, npwx2, G(n_start,1), nbnd )
        end if
        ...
        call start_clock('ppcg:dgemm')
        IF ( my_bgrp_id /= root_bgrp_id ) call threaded_memset( p, 0.d0, 2*npwx*nact ) 
        if (n_start .le. n_end) & ! could be done differently
        CALL DGEMM('N','N', npw2, nact, my_n,-ONE, psi(1,n_start), npwx2, G(n_start,1), nbnd, ONE, p, npwx2)
        CALL mp_sum( p(:,1:nact), inter_bgrp_comm )
        call stop_clock('ppcg:dgemm')
        !
        call start_clock('ppcg:dgemm')
        IF ( my_bgrp_id /= root_bgrp_id ) call threaded_memset( hp, 0.d0, 2*npwx*nact )

        if (n_start .le. n_end) &
        CALL DGEMM('N','N', npw2, nact, my_n,-ONE, hpsi(1,n_start), npwx2, G(n_start,1), nbnd, ONE, hp, npwx2)
        CALL mp_sum( hp(:,1:nact), inter_bgrp_comm )
        call stop_clock('ppcg:dgemm')
        !
        if (overlap) then
           call start_clock('ppcg:dgemm')
           IF ( my_bgrp_id /= root_bgrp_id ) call threaded_memset( sp, 0.d0, 2*npwx*nact )
           if (n_start .le. n_end) &
           CALL DGEMM('N','N', npw2, nact, my_n,-ONE, spsi(1,n_start), npwx2, G(n_start,1), nbnd, ONE, sp, npwx2)
     ...
     DO j = n_start, n_end
        ...
        IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, psi(1,(j-1)*sbsize + 1), npwx2, hpsi(1,(j-1)*sbsize + 1), &
                                                                                   npwx2, K, sbsize3 )
        !
        if (overlap) then
           CALL DGEMM('T','N', l, l, npw2, 2.D0, psi(1,(j-1)*sbsize + 1), npwx2, spsi(1,(j-1)*sbsize + 1), &
                                                                                    npwx2, 0.D0, M, sbsize3)
           IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, psi(1,(j-1)*sbsize + 1), npwx2, spsi(1,(j-1)*sbsize + 1), &
                                                                                            npwx2, M, sbsize3 )
        else
           CALL DGEMM('T','N', l, l, npw2, 2.D0, psi(1,(j-1)*sbsize + 1), npwx2, psi(1,(j-1)*sbsize + 1), &
                                                                                            npwx2, 0.D0, M, sbsize3)
           IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, psi(1,(j-1)*sbsize + 1), npwx2, psi(1,(j-1)*sbsize + 1), &
                                                                                           npwx2, M, sbsize3 )
        end if
        !
        ! ---
        CALL DGEMM('T','N', l, l, npw2, 2.D0, w(1,(j-1)*sbsize + 1), npwx2, hw(1,(j-1)*sbsize + 1), npwx2, &
                                                                                        0.D0, K(l+1, l+1), sbsize3)
        IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, w(1,(j-1)*sbsize + 1), npwx2, hw(1,(j-1)*sbsize + 1), &
                                                                                       npwx2, K(l+1, l+1), sbsize3 )
        !
        if (overlap) then
           CALL DGEMM('T','N', l, l, npw2, 2.D0, w(1,(j-1)*sbsize + 1), npwx2, sw(1,(j-1)*sbsize + 1), npwx2, &
                                                                                        0.D0, M(l+1, l+1 ), sbsize3)
           IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, w(1,(j-1)*sbsize + 1), npwx2, sw(1,(j-1)*sbsize + 1), &
                                                                                      npwx2, M(l+1, l+1), sbsize3 )
        else
           CALL DGEMM('T','N', l, l, npw2, 2.D0, w(1,(j-1)*sbsize + 1), npwx2, w(1,(j-1)*sbsize + 1), &
                                                                                    npwx2, 0.D0, M(l+1, l+1 ), sbsize3)
           IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, w(1,(j-1)*sbsize + 1), npwx2, w(1,(j-1)*sbsize + 1), &
                                                                                   npwx2, M(l+1, l+1), sbsize3 )
        end if
        !
        ! ---
        CALL DGEMM('T','N', l, l, npw2, 2.D0, psi(1,(j-1)*sbsize + 1), npwx2, hw(1,(j-1)*sbsize + 1), npwx2, 0.D0, &
                                                                                              K(1, l+1), sbsize3)
        IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, psi(1,(j-1)*sbsize + 1), npwx2, hw(1,(j-1)*sbsize + 1), &
                                                                                         npwx2, K(1, l+1), sbsize3 )
        !
        if (overlap) then
           CALL DGEMM('T','N', l, l, npw2, 2.D0, psi(1,(j-1)*sbsize + 1), npwx2, sw(1,(j-1)*sbsize + 1), &
                                                                                       npwx2, 0.D0, M(1, l+1), sbsize3)
           IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, psi(1,(j-1)*sbsize + 1), npwx2, sw(1,(j-1)*sbsize + 1), &
                                                                                         npwx2, M(1, l+1), sbsize3 )
        else
           CALL DGEMM('T','N', l, l, npw2, 2.D0, psi(1,(j-1)*sbsize + 1), npwx2, w(1,(j-1)*sbsize + 1), &
                                                                              npwx2, 0.D0, M(1, l+1), sbsize3)
           IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, psi(1,(j-1)*sbsize + 1), npwx2, w(1,(j-1)*sbsize + 1), &
                                                                                   npwx2, M(1, l+1), sbsize3 )
        end if
        call stop_clock('ppcg:dgemm')
        !
...
!ev        IF ( MOD(iter,rr_step) /= 1 ) THEN   ! In this case, P is skipped after each RR
        IF ( iter  /= 1 ) THEN
          call start_clock('ppcg:dgemm')
          CALL DGEMM('T','N', l, l, npw2, 2.D0, p(1,(j-1)*sbsize + 1), npwx2, hp(1,(j-1)*sbsize + 1), &
                                                                              npwx2, 0.D0, K(2*l + 1, 2*l+1), sbsize3)
          IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, p(1,(j-1)*sbsize + 1), npwx2, hp(1,(j-1)*sbsize + 1), &
                                                                                npwx2, K(2*l + 1, 2*l+1 ), sbsize3 )
          !
          if (overlap) then
             CALL DGEMM('T','N', l, l, npw2, 2.D0, p(1,(j-1)*sbsize + 1), npwx2, sp(1,(j-1)*sbsize + 1), &
                                                                              npwx2, 0.D0, M(2*l + 1, 2*l+1), sbsize3)
             IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, p(1,(j-1)*sbsize + 1), npwx2, sp(1,(j-1)*sbsize + 1), &
                                                                                 npwx2, M(2*l + 1, 2*l+1 ), sbsize3)
          else
             CALL DGEMM('T','N', l, l, npw2, 2.D0, p(1,(j-1)*sbsize + 1), npwx2, p(1,(j-1)*sbsize + 1), &
                                                                              npwx2, 0.D0, M(2*l + 1, 2*l+1), sbsize3)
             IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, p(1,(j-1)*sbsize + 1), npwx2, p(1,(j-1)*sbsize + 1), &
                                                                                   npwx2, M(2*l + 1, 2*l+1 ), sbsize3)
          end if
          !
          ! ---
          CALL DGEMM('T','N', l, l, npw2, 2.D0, psi(1,(j-1)*sbsize + 1), npwx2, hp(1,(j-1)*sbsize + 1), &
                                                                              npwx2, 0.D0, K(1, 2*l+1), sbsize3)
          IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, psi(1,(j-1)*sbsize + 1), npwx2, hp(1,(j-1)*sbsize + 1), &
                                                                                       npwx2, K(1, 2*l+1), sbsize3)
          !
          if (overlap) then
             CALL DGEMM('T','N', l, l, npw2, 2.D0, psi(1,(j-1)*sbsize + 1), npwx2, sp(1,(j-1)*sbsize + 1), &
                                                                                npwx2, 0.D0, M(1, 2*l+1), sbsize3)
             IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, psi(1,(j-1)*sbsize + 1), npwx2, sp(1,(j-1)*sbsize + 1), &
                                                                                           npwx2, M(1, 2*l+1), sbsize3)
          else
             CALL DGEMM('T','N', l, l, npw2, 2.D0, psi(1,(j-1)*sbsize + 1), npwx2, p(1,(j-1)*sbsize + 1), &
                                                                                   npwx2, 0.D0, M(1, 2*l+1), sbsize3)
             IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, psi(1,(j-1)*sbsize + 1), npwx2, p(1,(j-1)*sbsize + 1), &
                                                                                      npwx2, M(1, 2*l+1), sbsize3)
          end if
          call stop_clock('ppcg:dgemm')
          !
          ...
          !
          call start_clock('ppcg:dgemm')
          CALL DGEMM('T','N', l, l, npw2, 2.D0, w(1,(j-1)*sbsize + 1), npwx2, hp(1,(j-1)*sbsize + 1), npwx2, 0.D0, &
                                                                                      K(l+1, 2*l+1), sbsize3)
          IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, w(1,(j-1)*sbsize + 1), npwx2, hp(1,(j-1)*sbsize + 1), npwx2, &
                                                                                      K(l+1, 2*l+1), sbsize3)
          !
          if (overlap) then
             CALL DGEMM('T','N', l, l, npw2, 2.D0, w(1,(j-1)*sbsize + 1), npwx2, sp(1,(j-1)*sbsize + 1), &
                                                                                      npwx2, 0.D0, M(l+1, 2*l+1), sbsize3)
             IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, w(1,(j-1)*sbsize + 1), npwx2, sp(1,(j-1)*sbsize + 1), &
                                                                                      npwx2, M(l+1, 2*l+1), sbsize3)
          else
             CALL DGEMM('T','N', l, l, npw2, 2.D0, w(1,(j-1)*sbsize + 1), npwx2, p(1,(j-1)*sbsize + 1), &
                                                                                      npwx2, 0.D0, M(l+1, 2*l+1), sbsize3)
             IF ( gstart == 2 ) CALL DGER( l, l, -1.D0, w(1,(j-1)*sbsize + 1), npwx2, p(1,(j-1)*sbsize + 1), &
                                                                                      npwx2, M(l+1, 2*l+1), sbsize3)
          end if
          call stop_clock('ppcg:dgemm')
          !
...
!sdg      IF ( iter /= 1 ) THEN
       IF ( dimp == 3*l ) THEN
          ...
          !
          call start_clock('ppcg:dgemm')
          CALL DGEMM('N','N',npw2, l, l, ONE, p(1,(j-1)*sbsize + 1), npwx2, coord_p, sbsize, ZERO, buffer1, npwx2)
          CALL DGEMM('N','N', npw2, l, l, ONE, w(1,(j-1)*sbsize + 1), npwx2, coord_w, sbsize, ONE, buffer1, npwx2)
          p(:,(j-1)*sbsize + 1: (j-1)*sbsize + l)  = buffer1(:,1:l)
          call stop_clock('ppcg:dgemm')
          !
          call start_clock('ppcg:dgemm')
          CALL DGEMM('N','N',npw2, l, l, ONE, hp(1,(j-1)*sbsize + 1), npwx2, coord_p, sbsize, ZERO, buffer1, npwx2)
          CALL DGEMM('N','N', npw2, l, l, ONE, hw(1,(j-1)*sbsize + 1), npwx2, coord_w, sbsize, ONE, buffer1, npwx2)
          hp(:,(j-1)*sbsize + 1: (j-1)*sbsize + l)  = buffer1(:,1:l)
          call stop_clock('ppcg:dgemm')
          !
          if (overlap) then
             call start_clock('ppcg:dgemm')
             CALL DGEMM('N','N',npw2, l, l, ONE, sp(1,(j-1)*sbsize + 1), npwx2, coord_p, sbsize, ZERO, buffer1, npwx2)
             CALL DGEMM('N','N', npw2, l, l, ONE, sw(1,(j-1)*sbsize + 1), npwx2, coord_w, sbsize, ONE, buffer1, npwx2)
       ...
       ELSE
          !
          call start_clock('ppcg:dgemm')
          CALL DGEMM('N','N',npw2, l, l, ONE, w(1,(j-1)*sbsize + 1), npwx2, coord_w, sbsize, ZERO, buffer1, npwx2)
          p(:,(j-1)*sbsize + 1: (j-1)*sbsize + l) = buffer1(:, 1:l)
          call stop_clock('ppcg:dgemm')
          !
          call start_clock('ppcg:dgemm')
          CALL DGEMM('N','N',npw2, l, l, ONE, hw(1,(j-1)*sbsize + 1), npwx2, coord_w, sbsize, ZERO, buffer1, npwx2)
          hp(:,(j-1)*sbsize + 1: (j-1)*sbsize + l) = buffer1(:, 1:l)
          call stop_clock('ppcg:dgemm')
          !
          if (overlap) then
             call start_clock('ppcg:dgemm')
             CALL DGEMM('N','N',npw2, l, l, ONE, sw(1,(j-1)*sbsize + 1), npwx2, coord_w, sbsize, ZERO, buffer1, npwx2)
             sp(:,(j-1)*sbsize + 1: (j-1)*sbsize + l) = buffer1(:, 1:l)
             call stop_clock('ppcg:dgemm')
          end if
       END IF
       ...
       call start_clock('ppcg:dgemm')
       CALL DGEMM('N','N',npw2, l, l, ONE, psi(1,(j-1)*sbsize + 1), npwx2, coord_psi, sbsize, ZERO, buffer1, npwx2)
       psi(:, (j-1)*sbsize + 1: (j-1)*sbsize + l)  = buffer1(:,1:l) + p(:,(j-1)*sbsize + 1: (j-1)*sbsize + l)
       call stop_clock('ppcg:dgemm')
       !
       call start_clock('ppcg:dgemm')
       CALL DGEMM('N','N',npw2, l, l, ONE, hpsi(1,(j-1)*sbsize + 1), npwx2, coord_psi, sbsize, ZERO, buffer1, npwx2)
       hpsi(:, (j-1)*sbsize + 1: (j-1)*sbsize + l)  = buffer1(:,1:l) + hp(:,(j-1)*sbsize + 1: (j-1)*sbsize + l)
       call stop_clock('ppcg:dgemm')
       !
       if (overlap) then
          call start_clock('ppcg:dgemm')
          CALL DGEMM('N','N',npw2, l, l, ONE, spsi(1,(j-1)*sbsize + 1), npwx2, coord_psi, sbsize, ZERO, buffer1, npwx2)
...
!! EV begin
      ...
      IF ( .NOT. force_repmat ) THEN
         ...
         if (overlap) then
            CALL cholQR_dmat(npw, nact, psi, spsi, npwx, Gl, idesc)
         else
            CALL cholQR_dmat(npw, nact, psi, psi, npwx, Gl, idesc)
         end if
         call stop_clock('ppcg:cholQR')
         !
         call start_clock('ppcg:DTRSM')
         CALL dgemm_dmat( npw, nact, npwx, idesc, ONE, hpsi, Gl, ZERO, hpsi )
         call stop_clock('ppcg:DTRSM')
         !
         if (overlap) then
            call start_clock('ppcg:DTRSM')
            CALL dgemm_dmat( npw, nact, npwx, idesc, ONE, spsi, Gl, ZERO, spsi )
            call stop_clock('ppcg:DTRSM')
         end if
      ELSE
         !
         call start_clock('ppcg:cholQR')
         if (overlap) then
            CALL cholQR(npw, nact, psi, spsi, npwx, G, nbnd)
         else
            CALL cholQR(npw, nact, psi, psi, npwx, G, nbnd)
         end if
         call stop_clock('ppcg:cholQR')
         !
         call start_clock('ppcg:DTRSM')
         CALL DTRSM('R', 'U', 'N', 'N', npw2, nact, ONE, G, nbnd, hpsi, npwx2)
         call stop_clock('ppcg:DTRSM')
         !
         if (overlap) then
            call start_clock('ppcg:DTRSM')
            CALL DTRSM('R', 'U', 'N', 'N', npw2, nact, ONE, G, nbnd, spsi, npwx2)
            call stop_clock('ppcg:DTRSM')
         end if
      END IF
!! EV end
       ...
       CALL divide(inter_bgrp_comm,nact,n_start,n_end); my_n = n_end - n_start + 1; !write (*,*) nact,n_start,n_end
       if (n_start .le. n_end) &
       CALL DGEMM('T','N', nact, my_n, npw2, 2.D0, psi, npwx2, hpsi(1,n_start), npwx2, 0.D0, G(1,n_start), nbnd)
       IF ( gstart == 2 ) CALL DGER( nact, my_n, -1.D0, psi, npwx2, hpsi(1,n_start), npwx2, G(1,n_start), nbnd )
       CALL mp_sum(G(1:nact,1:nact), inter_bgrp_comm)
       !
       CALL mp_sum(G(1:nact,1:nact), intra_bgrp_comm)
       call stop_clock('ppcg:dgemm')
       ...
       ! w = hpsi - spsi*G
       IF ( my_bgrp_id /= root_bgrp_id ) then 
         call threaded_memset( w, 0.d0, 2*npwx*nact )
       ELSE
       ...
       if (overlap) then
          if (n_start .le. n_end) &
          CALL DGEMM('N','N',npw2, nact, my_n, -ONE, spsi(1,n_start), npwx2, G(n_start,1), nbnd, ONE, w, npwx2)
       else
          if (n_start .le. n_end) &
          CALL DGEMM('N','N',npw2, nact, my_n, -ONE, psi(1,n_start), npwx2, G(n_start,1), nbnd, ONE, w, npwx2)
       end if
    ...
    if(isgn.gt.0) then 
      do ibnd = 1, nact
        if(ibnd.ne.act_idx(ibnd)) then 
          buffer(:) = psi(:,ibnd) 
          psi(:,ibnd) = psi(:,act_idx(ibnd))
          psi(:,act_idx(ibnd)) = buffer(:) 
        end if 
      end do 
    elseif(isgn.lt.0) then 
      do ibnd = nact, 1, -1
        if(ibnd.ne.act_idx(ibnd)) then 
          buffer(:) = psi(:,act_idx(ibnd)) 
          psi(:,act_idx(ibnd)) = psi(:,ibnd)
          psi(:,ibnd) = buffer(:) 
        end if 
      end do 
    else
...
END SUBROUTINE ppcg_gamma_idx_acc

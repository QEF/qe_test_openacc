!
SUBROUTINE ppcg_k_idx_acc( h_psi_gpu, s_psi_gpu, overlap, precondition, &
...
!$acc parallel loop
          DO ii = 1, kdimx
            buffer(ii) = psi(ii,ibnd) 
            psi(ii,ibnd) = psi(ii,act_idx(ibnd))
            psi(ii,act_idx(ibnd)) = buffer(ii) 
          ENDDO
        end if 
      end do 
...
!$acc parallel loop
          DO ii = 1, kdimx
            buffer(ii) = psi(ii,act_idx(ibnd)) 
            psi(ii,act_idx(ibnd)) = psi(ii,ibnd)
            psi(ii,ibnd) = buffer(ii) 
          ENDDO 
        end if 
      end do 
    else

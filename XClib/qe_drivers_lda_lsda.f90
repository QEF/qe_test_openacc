...
MODULE qe_drivers_lda_lsda
  ...
  PRIVATE
  !
  PUBLIC :: xc_lda, xc_lsda, xc_lda_acc, xc_lsda_acc
  !
  !
CONTAINS
!
...
!
!-----------------------------------------------------------------------------
SUBROUTINE xc_lsda_acc( length, rho_in, zeta_in, ex_out, ec_out, vx_out, vc_out )
  !-----------------------------------------------------------------------------
  !! LSD exchange and correlation functionals - Hartree a.u.
  !
  !! * Exchange:
...
!$acc parallel loop
  DO ir = 1, length
     ...
     CASE( 3 )
        !
        CALL lsd_lyp( rho, zeta, ec, vc_up, vc_dw )       ! from CP/FPMD (more_functionals)
        !
     CASE( 4 )
        !
